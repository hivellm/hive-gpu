...
pub mod buffer_utils {
    ...
    pub fn validate_buffer_parameters(
        ...
        if size == 0 {
            return Err(HiveGpuError::BufferAllocationFailed("Zero buffer size".to_string()));
        }

        if size > max_size {
            return Err(HiveGpuError::BufferAllocationFailed(
                format!("Buffer size {} exceeds maximum {}", size, max_size)
            ));
        }

        if size % alignment != 0 {
            return Err(HiveGpuError::BufferAllocationFailed(
                format!("Buffer size {} not aligned to {}", size, alignment)
            ));
        }
