...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if let Some(retrieved_vector) = storage.get_vector("document_1")? {
        ...
        println!("   Title: {}", retrieved_vector.metadata.get("title").unwrap_or(&"N/A".to_string()));
        println!("   Category: {}", retrieved_vector.metadata.get("category").unwrap_or(&"N/A".to_string()));
    }
